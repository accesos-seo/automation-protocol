# 2026-05-04 14:06 UTC - Auditoría Estado Proyecto y Activación Controlada Bloqueada
...
Este handover corrige la ubicación de un handover previo que fue creado por error en:

```text
handover/automations/validation-shared-runtime-002-HANDOVER.md
```

La ubicación correcta para registros históricos operativos del proyecto es:

```text
handover/Registros_Historicos/2026/05-Mayo/
```

A partir de este registro, los handovers históricos deben incluir fecha y hora en el nombre del archivo para evitar ambigüedad cuando se generen muchos registros el mismo día.

Formato recomendado:

```text
YYYY-MM-DD_HH-MM_Titulo_Descriptivo.md
```

Ejemplo:

```text
2026-05-04_14-06_Auditoria_Estado_Proyecto_Activacion_Controlada_Bloqueada.md
```
...
[OK] handover histórico reubicado correctamente con hora en el título
```
...
```text
ruta = handover/Registros_Historicos/YYYY/MM-Mes/YYYY-MM-DD_HH-MM_Titulo_Descriptivo.md
usar hora en formato 24h
...
Organización: handovers históricos van en handover/Registros_Historicos/YYYY/MM-Mes/ con fecha y hora en el nombre.
```
